...
# for matrices
from numpy import zeros,savetxt, complex64,complex128, real, floor
...
class PairStateInteractions:
    ...
    def __loadAngularMatrixElementsFile(self):
        ...
        try:
            ...
            self.savedAngularMatrix_matrix = np.load(fileHandle,\
                                                     encoding = 'latin1', allow_pickle=True).tolist()
            fileHandle.close()
        except :
    ...
    def diagonalise(self,rangeR,noOfEigenvectors,
        ...
        for rval in self.r:
            ...
            # Add regularization to the Hamiltonian matrix
            import scipy
            reg_param = 1e-6
            m = m + reg_param * scipy.sparse.identity(m.shape[0])

            # Try diagonalizing with fewer eigenvectors
            try:
                ev, egvector = eigsh(m, k=min(100, m.shape[0] - 1), sigma=eigenstateDetuning * 1.e-9, which='LM', tol=1E-6)
            except RuntimeError as e:
                print("Error during diagonalization:", str(e))
                print("Attempting diagonalization with a different parameter set...")
                # Retry with a different sigma or tolerance if needed
                ev, egvector = eigsh(m, k=min(50, m.shape[0] - 1), sigma=None, which='LM', tol=1E-4)
